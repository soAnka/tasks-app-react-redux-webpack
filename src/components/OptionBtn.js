...
const OptionBtn = ({ menuType, value, className, methodToDispatch }) => {
    ...
    return (
        menuType==='toggle'?
        <StyledToggleBtn className={className} value={value} onChange={(e)=>dispatch(methodToDispatch(e.target.value))}>{value}</StyledToggleBtn> 
        :
        <StartBtn className={`${value}`} onClick={() =>dispatch(setUserChoice(value))}>{value}</StartBtn>
    ) 
}
